...
impl Database {
    ...
    fn apply_additive_migrations(&self) -> Result<()> {
        ...
        // T-P3.4 PR 3: same shape collapse for static-analysis findings
        // — static_analysis_findings, static_analysis_finding_attribution,
        // static_analysis_runs all lose sprint_id; UNIQUE on findings
        // becomes (repo_full_name, fingerprint); runs PK becomes
        // (repo_full_name, analyzer). Detect on the findings table.
        let saf_cols = self
            .column_names("static_analysis_findings")
            .unwrap_or_default();
        if saf_cols.iter().any(|c| c == "sprint_id") {
            self.conn.execute(
                "DROP TABLE IF EXISTS static_analysis_finding_attribution",
                [],
            )?;
            self.conn
                .execute("DROP TABLE IF EXISTS static_analysis_findings", [])?;
            self.conn
                .execute("DROP TABLE IF EXISTS static_analysis_runs", [])?;
            self.conn
                .execute("DROP INDEX IF EXISTS idx_sa_findings_sprint", [])?;
            self.conn
                .execute("DROP INDEX IF EXISTS idx_sa_attr_sprint", [])?;
        }
...
mod tests {
    ...
    /// Regression for T-P3.4 PR 3: a DB carrying the legacy per-sprint
    /// static-analysis shape (sprint_id in static_analysis_findings)
    /// must be rewritten to the artifact-level shape on the next
    /// `create_tables` call.
    #[test]
    fn migration_drops_legacy_per_sprint_static_analysis_shape() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("grading.db");
            ...
            conn.execute_batch(
                "CREATE TABLE static_analysis_findings (
                 ...
                 INSERT INTO static_analysis_findings
                    (repo_full_name, sprint_id, analyzer, rule_id, severity,
                     file_path, message, fingerprint)
                    VALUES ('udg-pds/spring-x', 70, 'pmd', 'UnusedField',
                            'WARNING', 'Foo.java', 'unused', 'fp-1');",
            )
            .unwrap();
        }
        ...
        let cols = db.column_names("static_analysis_findings").unwrap();
        assert!(
            !cols.iter().any(|c| c == "sprint_id"),
            "legacy sprint_id column must be dropped, got: {cols:?}"
        );
        ...
        let attr_cols = db
            .column_names("static_analysis_finding_attribution")
            .unwrap();
        assert!(
            !attr_cols.iter().any(|c| c == "sprint_id"),
            "attribution sprint_id must be dropped, got: {attr_cols:?}"
