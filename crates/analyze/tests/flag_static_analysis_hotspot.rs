//! STATIC_ANALYSIS_HOTSPOT — per-student artifact flag (T-SA / T-P3.4).
//! Sums each student's blame-attribution `weight` across the project's
//! `static_analysis_findings` rows; fires when the sum is ≥
//! `detector_thresholds.static_analysis_hotspot_min_weighted` (default
//! `10.0`). Sprint-free: the flag lives in `student_artifact_flags`.
...
fn insert_finding(
    ...
    conn.execute(
        "INSERT INTO static_analysis_findings
            (repo_full_name, analyzer, rule_id, severity, category, file_path,
             start_line, end_line, message, fingerprint)
         VALUES ('udg/x', ?, ?, ?, ?, ?, 1, 5, 'msg', ?)",
        params![analyzer, rule_id, severity, category, file, fingerprint],
    )
    .unwrap();
...
fn default_threshold_keeps_flag_silent() {
    ...
    common::seed_default_project(&conn);
    common::seed_student(&conn, "alice");
    let f1 = insert_finding(&conn, "A.java", "R", "WARNING", "pmd", "style");
    insert_attribution(&conn, f1, "alice", 5.0);

    detect_artifact_flags_for_project_id(&conn, common::PROJECT_ID, &Config::test_default())
        .unwrap();
    assert_eq!(
        common::count_artifact_flags(&conn, common::PROJECT_ID, "STATIC_ANALYSIS_HOTSPOT"),
        0,
...
fn worst_severity_propagates_to_flag() {
    ...
    common::seed_default_project(&conn);
    common::seed_student(&conn, "alice");
    let f1 = insert_finding(&conn, "A.java", "R1", "INFO", "checkstyle", "style");
    let f2 = insert_finding(&conn, "B.java", "R2", "CRITICAL", "spotbugs", "security");
...
fn each_student_evaluated_independently() {
    ...
    insert_attribution(&conn, f2, "bob", 0.5);
